...
1. The plugin registers hooks across Claude Code's interaction points: turn starts
   ...
   (`AskUserQuestion`), permission requests, MCP input requests (`Elicitation`), prompt
   resolutions (`PostToolUse`, `PermissionDenied`, and `ElicitationResult`, used to
   auto-clear a prompt once you answer — or deny — it in the terminal), idle
...
4. You answer the prompt in your terminal as usual. Clicking the banner (or the "Focus"
   ...
   Once you answer, AgentBar notices the session make progress (`PostToolUse` / `Stop` —
   or `PermissionDenied` / `ElicitationResult` when you deny or respond directly) and
...
│   ├── hooks/hooks.json               # UserPromptSubmit / PreToolUse / PermissionRequest / PermissionDenied / PostToolUse / Elicitation / ElicitationResult / Notification / Stop / SubagentStop / SessionEnd / StopFailure
